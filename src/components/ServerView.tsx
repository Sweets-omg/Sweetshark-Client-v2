...
export function ServerView({ server, refreshTrigger }: ServerViewProps) {
  ...
  return (
    <div className="w-full h-full bg-[var(--window-bg)] relative">
      {/* Loading overlay */}
      {isLoading && !hasError && (
        <div className="absolute inset-0 flex items-center justify-center bg-[var(--window-bg)] z-10">
      ...
      {hasError && (
        <div className="absolute inset-0 flex items-center justify-center bg-[var(--window-bg)] z-10">
          <div className="text-center space-y-4 max-w-md p-8">
            ...
            <button
              onClick={handleRetry}
              className="bg-[var(--accent)] text-white px-6 py-2 rounded hover:bg-[var(--accent-hover)] transition-colors"
            >
              Retry Connection
            </button>
          </div>
